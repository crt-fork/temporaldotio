...
export default function EmailForm({ data }) {
  const MAIL_CHIMP_URL = `https://temporal.us17.list-manage.com/subscribe/post?u=2334a0f23e55fd1840613755d&amp;id=3475f910fc`;

  const Message = ({ status, message, className }) => (
    <p
      role="alert"
      className={clsx(
        `text-sm px-3 mt-1 ${className}`,
    ...
    return (
      <div className="mb-8 md:mb-32 relative">
        ...
        >
          ...
          <button
            className={clsx(
              'self-stretch text-center text-black rounded-r-lg rounded-l-none bg-blue-100 border hover:text-white hover:bg-blue-700 pl-4 pr-5 flex cursor-pointer'
            )}
        ...
        {!!messageState && (
          <Message
            className="block absolute w-full bottom-[-34px] text-center pt-2.5"
