...
export default function Footer({ footer }) {
  ...
  return (
    <footer className='pt-24 pb-48 bg-black text-white overflow-x-hidden'>
              ...
              {col.links.map((link, i) => (
                <li className='text-left' key={`footer-col-link-${index}-${i}`}>
                  <Link
                    href={link.href || ''}
                    target={link.target}>
                    <a className='text-left font-sans font-thin hover:text-blue-200 focus:text-blue-200'>
                      {link.copy}
                    </a>
                  </Link>
                </li>
      ...
      <div className='container max-w-screen-xl w-full mx-auto px-10 flex flex-wrap gap-x-6 flex-wrap xl:flex-nowrap justify-between mt-5 capitalize'>
          <div className='flex basis-full grow-0 shrink-0 xl:basis-auto xl:order-2 flex-wrap justify-center'>
            <p className='w-full pb-5'>Backed by</p>
            {partners.map((item, i) => (
              <div
          ...
          <div className='mt-20 xl:mt-0 flex basis-full shrink-0 grow-0 flex-wrap gap-8 xl:basis-auto xl:gap-4 justify-center xl:order-1 xl:justify-left'>
              ...
              if (item.href) {
                return (
                  <div key={`footer-legal-items-${i}`}
                    ...
                    <Link
                      href={item.href || ''}>
                      <a className='text-left font-sans font-thin hover:text-blue-200'>
                        {item.copy}
                      </a>
                    </Link>
                  </div>
