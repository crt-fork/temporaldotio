    ...
    <header>
      <nav className="w-full py-[1.375rem] bg-white shadow-lg px-10 relative">
        <div className="container-fluid w-full flex flex-wrap items-center justify-between capitalize">
          ...
          <div
            ...
            <div className='flex-initial ml-10'>
              <Link
                href={nav.cta.href} passHref>
                  <a className="py-[5px] px-[16px] border rounded-md border-black text-sm hover:border-white hover:bg-black hover:text-white" target={nav.cta.target}>
                    {nav.cta.copy}
                  </a>
              </Link>
            </div>
        ...
        <div
          ...
          (activeBarRect === 'hidden') ? 'hidden' : 'block')}
          style={{
